...
# ### Method: Preserving Onset and Window = 700ms
...
# epochs = 5  # For quick performance testing
...
# conv1D_3 = [256,9,1] # number_filters,kernel_size and strides
...
for loo_subject in range(num_subjects):  # Change range to start from 0 to num_subjects-1
	...
	# Initialising Wandb logging
	wandb.init(project='Thesis', entity='firass-koli', config=config, name=f"loo_test_subject_{loo_subject + 1}",
	           group="Inter")
	optimizer = Adam(learning_rate=wandb.config.learning_rate)
	...
	plotting_loss_and_accuracy_over_epochs(history_name,
	                                       f'Loss and Accuracy over Epochs (Interpersonal with Subject_{loo_subject + 1} as loo)')
	...
	plot_confusion_matrix(x_train, y_train, 'Greens',
	                      f'Confusion Matrix of Interpersonal Training with Subject_{loo_subject + 1} as loo')
	# Confusion Matrix in Testing
	interpersonal_precision, interpersonal_recall, interpersonal_f1, interpersonal_f1_micro = \
		plot_confusion_matrix(x_val, y_val, 'Blues',
		                      f'Confusion Matrix of Interpersonal LOO_Validation on Subject_{loo_subject + 1}')
...
def plot_interpersonal_metrics_bar(all_interpersonal_precision, all_interpersonal_recall, all_interpersonal_f1_list,
	...
	plt.savefig(f"results_figures/interpersonal_test_scores_{round(np.mean(all_interpersonal_f1_micro_list), 3)}.png")
	plt.show()
